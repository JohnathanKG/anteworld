...
ot::vehicle_params simplugin::init_chassis(const coid::token& params)
{
	...
	wheelParam->suspension_max = 0.1f; // max. movement up from default position [m]
	wheelParam->suspension_min = -0.12f; // max. movement down from default position [m]
	wheelParam->suspension_stiffness = 30.0f; // suspension stiffness coefficient
	wheelParam->damping_compression = 0.4f; // damping coefficient for suspension compression
	wheelParam->damping_relaxation = 0.12f; // damping coefficient for suspension relaxation
	wheelParam->grip = 1.f; // relative tire grip compared to an avg. tire, +-1
	wheelParam->slip_lateral_coef = 1.5f; // lateral slip multiplier, relative to the computed longitudinal tire slip
	wheelParam->differential = true; // true if the wheel is paired with another through a differential
	...
	vp->steering.bone_ovr = "steering_wheel"; // steering wheel bone name override, default "steering_wheel"
	vp->steering.radius = 0.17f; // steering wheel radius, if 0 disabled
	vp->steering.grip_angle = 0.15f; // grip angular offset in degrees
	vp->steering.steering_thr = 50.f; // speed [km/h] at which the steering speed is reduced by 60%
	vp->steering.centering_thr = 20.f; // speed [km/h] when the centering acts at 60% already
...
void simplugin::update_frame(float dt, float throttle, float brake, float steering, float parking)
{
